...
import { data as setTimezone } from '../src/commands/set-timezone.js';
import { data as assignTask } from '../src/commands/assign-task.js';
import { logger } from '../src/utils/logger.js';

async function register() {
  const rest = new REST({ version: '10' }).setToken(config.DISCORD_TOKEN);
  const cmds = [myinfo.toJSON(), setTimezone.toJSON(), assignTask.toJSON()];
